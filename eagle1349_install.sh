...
ADMIN_EMAIL="info@eagle-erp.com"
##
...
sudo -H pip3 install -r https://github.com/rapidgrps/eagle13.03/raw/${OE_VERSION}/requirements.txt
...
if [ $INSTALL_WKHTMLTOPDF = "True" ]; then
  echo -e "\n---- Install wkhtml and place shortcuts on correct place for Eagle ERP 13.03 ----"
...
sudo git clone --depth 1 --branch $OE_VERSION https://github.com/rapidgrps/eagle13.03 $OE_HOME_EXT/
...
EOF
  ...
  sudo mv ~/eagle1349 /etc/nginx/sites-available/
  sudo ln -s /etc/nginx/sites-available/eagle1349 /etc/nginx/sites-enabled/eagle1349
  sudo rm /etc/nginx/sites-enabled/default
  sudo service nginx reload
  sudo su root -c "printf 'proxy_mode = True\n' >> /etc/${OE_CONFIG}.conf"
  echo "Done! The Nginx server is up and running. Configuration can be found at /etc/nginx/sites-available/eagle1349"
else
...
if [ $INSTALL_NGINX = "True" ]; then
  echo "Nginx configuration file: /etc/nginx/sites-available/eagle"
fi
echo "-----------------------------------------------------------"
